...
const Cart = () => {
  ...
  const amountBeforeTax = cart?.cartItems.reduce(
    (acc, item) => acc + item.quantity * item.price,
    0
  );

  const tax = Number(amountBeforeTax * 0.14).toFixed(2);

  const totalAmount = (Number(amountBeforeTax) + Number(tax)).toFixed(2);
